let price = 3.26;
let cid = [
  ['PENNY', 1.01],
  ['NICKEL', 2.05],
  ['DIME', 3.1],
  ['QUARTER', 4.25],
  ['ONE', 90],
  ['FIVE', 55],
  ['TEN', 20],
  ['TWENTY', 60],
  ['ONE HUNDRED', 100]
];

const displayChangeDue = document.getElementById('change-due');
const cash = document.getElementById('cash');
const purchaseBtn = document.getElementById('purchase-btn');
const priceScreen = document.getElementById('price-screen');
const cashDrawerDisplay = document.getElementById('cash-drawer-display');

const currencyNameMap = {
  PENNY: 'Pennies',
  NICKEL: 'Nickels',
  DIME: 'Dimes',
  QUARTER: 'Quarters',
  ONE: 'Ones',
  FIVE: 'Fives',
  TEN: 'Tens',
  TWENTY: 'Twenties',
  'ONE HUNDRED': 'Hundreds'
};

// const updateUI = (change) => {
//   if (change) {
//     change.forEach(([currency, amount]) => {
//       const targetArr = cid.find((cidArr) => cidArr[0] === currency);
//       targetArr[1] = parseFloat((targetArr[1] - amount).toFixed(2));
//     });
//   }

const updateUI = (change) => {
  const updatedCid = [...cid]; // Create a copy
  if (change) {
    change.forEach(([currency, amount]) => {
      const targetArr = updatedCid.find((cidArr) => cidArr[0] === currency);
      targetArr[1] = parseFloat((targetArr[1] - amount).toFixed(2));
    });
  }

  cash.value = '';
  priceScreen.textContent = `Total: $${price}`;
  cashDrawerDisplay.innerHTML = `<p><strong>Change in drawer (CID):</strong></p>
    ${cid
      .map(([currency, amount]) => `<p>${currencyNameMap[currency]}: $${amount}</p>`)
      .join('')}  
  `;
};

const formatResults = (status, change) => {
  displayChangeDue.innerHTML = `<p>Status: ${status}</p>`;
  change.forEach(([currency, amount]) => {
    displayChangeDue.innerHTML += `<p>${currency}: $${amount}</p>`;
  });
};

const handleInsufficientFunds = () => {
  displayChangeDue.innerHTML = '<p>Status: INSUFFICIENT_FUNDS</p>';
};

const checkCashRegister = () => {

  const cashValue = Number(cash.value);

  if (cashValue < price) {
    alert('Customer does not have enough money to purchase the item');
    cash.value = '';
    return;
  }

  if (cashValue === price) {
    displayChangeDue.innerHTML = '<p>No change due - customer paid with exact cash</p>';
    cash.value = '';
    return;
  }

  let changeDue = cashValue - price;
  const reversedCid = [...cid].reverse();
  const denominations = [100, 20, 10, 5, 1, 0.25, 0.1, 0.05, 0.01];
  const result = { status: 'OPEN', change: [] };

  // let totalCID = parseFloat(cid.map(([_, total]) => total).reduce((prev, curr) => prev + curr).toFixed(2));

  let totalCID = parseFloat(cid.reduce((total, [_, amount]) => total + amount, 0).toFixed(2));


  if (totalCID < changeDue) {
    return handleInsufficientFunds();
  }

  if (totalCID === changeDue) {
    return formatResults('CLOSED', cid);
  }

  for (let i = 0; i < reversedCid.length; i++) {
    if (changeDue > denominations[i] && changeDue > 0) {
      let count = 0;
      let total = reversedCid[i][1];
      while (total > 0 && changeDue >= denominations[i]) {
        total -= denominations[i];
        changeDue = parseFloat((changeDue - denominations[i]).toFixed(2));
        count++;
      }
      result.change.push([reversedCid[i][0], count * denominations[i]]);
    }
  }
  
 
 if (changeDue > 0) {
    return handleInsufficientFunds();
  }

  formatResults(result.status, result.change);
  updateUI(result.change);
};

const checkResults = () => {
  if (!cash.value || isNaN(Number(cash.value))) {
    return;
  }
  checkCashRegister();
};

purchaseBtn.addEventListener('click', checkResults);

cash.addEventListener('keydown', (e) => {
  if (e.key === 'Enter') {
    checkResults();
  }
});

updateUI();
